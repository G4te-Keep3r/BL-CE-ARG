...
lasts = [] #in the context of this script, lasts is a bad name, right now this is an experiment off of a less persistent one...which is an experiment in its own
...
def make_img(d, i):
	...
	#if x,y has ever been set to something, and is not set above, make it "blank"(/silver)
	for cord in lasts:
		ca = list(cord) #"cord array"
	...
	with open('data_1-black_0-white.csv') as f:
		for line in f:
			...
			for i in range(len(bs)):
				...
				#persistence
				if tempcord not in lasts:
					lasts.append(tempcord)
					#i know this only needs to be done the first 5 times, but dont want to do it before each is used and this works
